...
bool Bot::LookupEnemy (void)
{
   ...
   if (!IsNullEntity (m_enemy) && m_enemyUpdateTime > GetWorldTime ())
   {
      player = m_enemy;
   ...
   if (IsValidPlayer (newEnemy))
   {
      ...
      {
         if (m_seeEnemyTime + 3.0 < GetWorldTime () && (m_hasC4 || HasHostage () || !IsNullEntity (m_targetEntity)))
         ...
         for (int j = 0; j < GetMaxClients (); j++)
         {
            ...
            if (friendBot != NULL)
            {
               if (friendBot->m_seeEnemyTime + 2.0f < GetWorldTime () || IsNullEntity (friendBot->m_lastEnemy))
               {
   ...
   if ((m_aimFlags <= AIM_PREDICT_PATH && m_seeEnemyTime + 3.0 < GetWorldTime () &&  !(m_states & (STATE_SEEING_ENEMY | STATE_HEARING_ENEMY)) && IsNullEntity (m_lastEnemy) && IsNullEntity (m_enemy) && GetTaskId () != TASK_SHOOTBREAKABLE && GetTaskId () != TASK_PLANTBOMB && GetTaskId () != TASK_DEFUSEBOMB) || g_roundEnded)
   {
      if (!m_reloadState)
...
void Bot::FireWeapon (void)
{
   ...
   if (m_difficulty >= 3 && pev->health > 80 && !IsNullEntity (enemy) && pev->health >= enemy->v.health && distance < 100.0f && !IsGroupOfEnemies (pev->origin))
      goto WeaponSelectEnd;
...
WeaponSelectEnd:
   ...
   if (UsesSniper () && m_zoomCheckTime < GetWorldTime ()) // is the bot holding a sniper rifle?
   {
      ...
      if (!IsNullEntity (m_enemy) && (pev->velocity.x != 0 || pev->velocity.y != 0 || pev->velocity.z != 0) && (pev->basevelocity.x != 0 || pev->basevelocity.y != 0 || pev->basevelocity.z != 0))
      {
         m_moveSpeed = 0.0;
         m_strafeSpeed = 0.0;
